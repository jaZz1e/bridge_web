...
(function ($) {
    $.fn.mobilymap = function (options) {
        ...
        return this.each(function () {
            ...
            var content = $this.find(".imgContent"), image = $this.find("img"), title = image.attr("alt"),
                point = $this.find("." + sets.markerClass), mouseDown = false, mx, my, ex, ey, imgw = image.width(),
                imgh = image.height(), divw = $this.width(), divh = $this.height();
            var cookies = {
            ...
            var map = {
                check: function (x, y) {
                    ...
                    switch (position) {
                        case"center":
                            var x = (divw - imgw) / 2, y = (divh - imgh) / 2;
                            break;
                        case"top left":
                            var x = 0, y = 0;
                            break;
                        ...
                        case"bottom left":
                            ...
                            break;
                        case"bottom right":
                            var x = divw - imgw, y = divh - imgh;
                            break;
                        default:
                            var new_position = position.split(" "), x = -(new_position[0]), y = -(new_position[1]);
                            if (y < (divh - imgh)) {
                                y = divh - imgh
                            } else {
                                if (y > 0) {
                    ...
                    if (sets.cookies) {
                        if (cookies.read("position") != null) {
                            var pos = cookies.read("position").split(","), x = pos[0], y = pos[1]
                        } else {
                            var x = (divw - imgw) / 2, y = (divh - imgh) / 2
                        }
                    }
                    ...
                    var loadimg = new Image(), src = image.attr("src");
                    ...
                    var mouse = map.mouse(e), x = mouse.x, y = mouse.y, movex = x - mx, movey = y - my,
                        top = ey + movey, left = ex + movex, check = map.check(left, top);
                    ...
                    buttons: function () {
                        ...
                        $("." + sets.navBtnClass).bind({
                            mousedown: function () {
                                ...
                                if (navbtn == 0) {
                                    content.animate({top: 0}, sets.navSpeed)
                                }
                                if (navbtn == 1) {
                                    content.animate({left: divw - imgw + "px"}, sets.navSpeed)
                                }
                                if (navbtn == 2) {
                                    content.animate({top: divh - imgh + "px"}, sets.navSpeed)
                                }
                                if (navbtn == 3) {
                                    content.animate({left: 0}, sets.navSpeed)
                                }
                            }, mouseup: function () {
                                content.stop();
                                var pos = content.position(), x = pos.left, y = pos.top;
                                if (sets.cookies) {
            ...
            content.bind({
                mousedown: function (e) {
                    ...
                    if (sets.caption) {
                        captiond.animate({bottom: -captionh + "px"})
                    }
                    return false
                }, mouseleave: function () {
                    if (sets.caption) {
                        captiond.animate({bottom: 0})
                    }
                    return false
                }
            });
            ...
            point.click(function () {
                var $this = $(this), pointw = $this.width(), pointh = $this.height(), pos = $this.position(),
                    py = pos.top, px = pos.left, wrap = $this.find(".markerContent").html();
                if (sets.setCenter) {
                    var center_y = -py + divh / 2 - pointh / 2, center_x = -px + divw / 2 - pointw / 2,
                ...
                if (sets.popup) {
                    ...
                    var popup = $this.next("." + sets.popupClass), popupw = popup.innerWidth(),
                        popuph = popup.innerHeight(), y = py, x = px;
                    popup.css({top: y + pointh + "px", left: x + "px", marginLeft: -(popupw / 2 - pointw / 2) + "px"})
                } else {
